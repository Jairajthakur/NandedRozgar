...
// FIX #9 — rejectUnauthorized must be TRUE in production to prevent MITM attacks
const pool = new Pool({
...
async function runMigrations() {
  ...
  try {
    ...
    // FIX #14 — Audit log table for all admin actions
    await client.query(`
    ...
    // FIX #2 — Seed admin from environment variables only, no hardcoded credentials.
    // FIX #13 — Never log credentials.
    // Set ADMIN_EMAIL and ADMIN_PASSWORD in your environment before first deploy.
    ...
    // FIX #2 — Removed hardcoded test user seed entirely.
