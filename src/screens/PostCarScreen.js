...
export default function PostCarScreen() {
  ...
  async function openCamera() {
    ...
    if (!result.canceled && result.assets?.length) {
      if (photos.length >= 8) { Alert.alert('Limit reached', 'You can upload up to 8 photos.'); return; }
  ...
  async function openGallery() {
    ...
    if (remaining <= 0) { Alert.alert('Limit reached', 'You can upload up to 8 photos.'); return; }
    ...
    if (!result.canceled && result.assets?.length) {
      setPhotos(p => [...p, ...result.assets.map(a => a.uri)].slice(0, 8));
    }
  }
  ...
  function handleSubmit() {
    if (!form.name || !form.dailyRate || !form.area || !form.whatsapp) {
      Alert.alert('Missing fields', 'Please fill vehicle name, daily rate, area and WhatsApp number.');
      return;
    }
    if (photos.length === 0) {
      ...
      <ScrollView style={s.container} contentContainerStyle={{ paddingBottom: 50 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.header}>
          ...
          <View>
            ...
            <Text style={s.headerSub}>Fill details to attract renters</Text>
          </View>
        </View>
        ...
        <View style={s.body}>
          ...
          {/* ── SECTION 1: Photos ── */}
          <Section emoji="📷" title="Vehicle Photos">
            <View style={s.tipBox}>
              <Ionicons name="information-circle" size={15} color="#2563eb" />
              <Text style={s.tipTxt}>Listings with <Text style={{ fontWeight: '800' }}>3+ photos</Text> get 4× more bookings. Add front, side, interior &amp; back.</Text>
            </View>
          ...
          <Section emoji="🚗" title="Vehicle Details">
          ...
          <Section emoji="💰" title="Pricing & Booking">
          ...
          <Section emoji="📍" title="Location & Contact">
            ...
            <TextInput
              ...
              placeholder="Special conditions, rules, or extra info for renters…"
          ...
          {/* ── SECTION 5: Listing Plan ── */}
          <Section emoji="📅" title="Choose Listing Plan">
            <Text style={s.planNote}>Listing auto-expires after the plan period ends. One flat fee covers everything.</Text>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            <Text style={s.submitTxt}>{loading ? 'Processing…' : '🚗  List My Vehicle'}</Text>
...
const s = StyleSheet.create({
  ...
  // Tips
  tipBox: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, backgroundColor: '#eff6ff', borderRadius: 10, padding: 11, marginBottom: 14 },
