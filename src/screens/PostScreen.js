...
export default function PostScreen({ navigation }) {
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        ...
        <Text style={{ fontSize: 13, color: C.muted, textAlign: 'center' }}>
          Job posting is only for employers.
        </Text>
  ...
  const [form, setForm] = useState({
    title: '', company: user?.company || '', category: 'Construction',
    type: 'Full-time', location: '', salary: '', phone: user?.phone || '',
    description: '', featured: false, urgent: false,
  });
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [showPayModal, setShowPayModal] = useState(false);
  const [payForm, setPayForm] = useState({ card: '', expiry: '', cvv: '', name: user?.name || '' });

  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const setP = (k, v) => setPayForm(f => ({ ...f, [k]: v }));

  const boostPrice = (form.featured && form.urgent) ? PRICING.bundle
  ...
  async function processPayment() {
    ...
    const boostLabel = form.featured && form.urgent ? ' + Featured + Urgent'
                     : form.featured ? ' + Featured' : form.urgent ? ' + Urgent' : '';
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1, backgroundColor: C.bg }}
          ...
          <Card style={{ marginBottom: 16 }}>
            <Input label="Cardholder Name" value={payForm.name}
              onChangeText={v => setP('name', v)} placeholder="Name on card" />
            <Input label="Card Number" value={payForm.card}
              onChangeText={v => setP('card', v)} placeholder="1234 5678 9012 3456"
              keyboardType="number-pad" maxLength={19} />
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <View style={{ flex: 1 }}>
                <Input label="Expiry (MM/YY)" value={payForm.expiry}
                  onChangeText={v => setP('expiry', v)} placeholder="MM/YY" maxLength={5} />
              </View>
              <View style={{ flex: 1 }}>
                <Input label="CVV" value={payForm.cvv}
                  onChangeText={v => setP('cvv', v)} placeholder="•••"
                  maxLength={3} secureTextEntry />
              </View>
            </View>
            {error ? <Text style={styles.error}>{error}</Text> : null}
            <Btn label={loading ? 'Processing…' : `🔒 Pay ₹${total} Securely`}
              onPress={processPayment} disabled={loading} size="lg" style={{ marginTop: 4 }} />
            <Text style={styles.sslText}>🔒 256-bit SSL · Powered by Razorpay · PCI DSS</Text>
            <Btn label="Cancel" variant="outline" onPress={() => setShowPayModal(false)}
              style={{ marginTop: 8 }} />
          </Card>
        </ScrollView>
      ...
      <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
        ...
        <Card style={{ marginBottom: 16 }}>
          <Input label="Job Title *" value={form.title}
            onChangeText={v => set('title', v)} placeholder="e.g. Security Guard Needed" />
          <Input label="Company Name *" value={form.company}
            onChangeText={v => set('company', v)} placeholder="Your company or name" />

          {/* Category picker */}
          <Text style={styles.fieldLabel}>Category *</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}
            style={{ marginBottom: 14 }} contentContainerStyle={{ gap: 6 }}>
          ...
          <Input label="Location in Nanded *" value={form.location}
            onChangeText={v => set('location', v)} placeholder="e.g. Cidco, Shivaji Nagar" />
          <Input label="Salary / Pay *" value={form.salary}
            onChangeText={v => set('salary', v)} placeholder="e.g. ₹12,000/month or ₹600/day" />
          <Input label="Contact Phone *" value={form.phone}
            onChangeText={v => set('phone', v)} placeholder="10-digit mobile"
            keyboardType="phone-pad" maxLength={10} />
          <Input label="Job Description *" value={form.description}
            onChangeText={v => set('description', v)}
            placeholder="Describe the job, requirements, timings…"
            multiline numberOfLines={4} inputStyle={{ minHeight: 100, textAlignVertical: 'top' }} />
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 14 }}>
            {[
              ...
              <TouchableOpacity key={key} onPress={() => set(key, !form[key])}
          ...
          <Btn label={`🚀 Post Job & Pay ₹${total}`} onPress={handlePost} size="lg" style={{ marginTop: 4 }} />
        </Card>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  boostBtn: {
    flex: 1, borderWidth: 2, borderColor: C.border, borderRadius: 11,
  ...
  payTitle: { color: 'rgba(255,255,255,0.7)', fontSize: 12, fontWeight: '700',
