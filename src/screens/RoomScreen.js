          ...
          <View style={s.tagsRow}>
            {item.type && (
              <View style={s.tag}>
                ...
                <Text style={s.tagTxt}>{item.type}</Text>
              </View>
            )}
            {item.for && item.for !== 'Any' && (
              <View style={s.tag}>
                <Ionicons name="people-outline" size={10} color="#555" style={{ marginRight: 3 }} />
                <Text style={s.tagTxt}>{item.for}</Text>
              </View>
            )}
            {item.available && (
              <View style={[s.tag, s.tagGreen]}>
                <Ionicons name="checkmark-circle" size={11} color="#16a34a" style={{ marginRight: 3 }} />
                <Text style={[s.tagTxt, { color: '#16a34a' }]}>Available</Text>
              </View>
            )}
            {item.owner?.verified && (
          ...
          <View style={s.cardFooter}>
            <View style={s.ownerRow}>
              <View style={s.ownerAvatar}>
                <Text style={s.ownerInitial}>{(item.owner?.name || 'O')[0].toUpperCase()}</Text>
              </View>
    ...
    return (
      <View style={ws.root}>
        <View style={ws.sidebar}>
          <View style={ws.sideCard}>
            <Text style={ws.sideTitle}>RENT RANGE</Text>
            {RENT_RANGES.map(rr => (
              <TouchableOpacity key={rr.label} onPress={() => setRentRange(rr)} style={[ws.sortRow, rentRange.label === rr.label && ws.sortRowActive]}>
                <Text style={[ws.sortTxt, rentRange.label === rr.label && { color: ORANGE, fontWeight: '700' }]}>{rr.label}</Text>
                {rentRange.label === rr.label && <Ionicons name="checkmark-circle" size={14} color={ORANGE} />}
              </TouchableOpacity>
            ))}
          </View>
          <View style={[ws.sideCard, ws.ctaCard]}>
            <View style={ws.ctaCircle1} /><View style={ws.ctaCircle2} />
            <Text style={ws.ctaEyebrow}>HAVE A ROOM?</Text>
            <Text style={ws.ctaTitle}>List your room free</Text>
            <Text style={ws.ctaSub}>Reach 10,000+ seekers in Nanded</Text>
            <TouchableOpacity style={ws.ctaBtn} onPress={() => nav.navigate('PostRoom')}>
              <Ionicons name="add-circle-outline" size={14} color="#fff" />
              <Text style={ws.ctaBtnTxt}>Post Room</Text>
            </TouchableOpacity>
          </View>
          <View style={[ws.sideCard, ws.tipCard]}>
            <Text style={ws.tipTitle}>RENTING TIPS</Text>
            {['Verify owner identity before deposit', 'Visit in person before finalising', 'Check water & electricity supply'].map((tip, i) => (
        ...
        <View style={ws.sidebar}>
          <View style={ws.sideCard}>
            ...
            {SORT_OPTIONS.map(opt => (
              <TouchableOpacity key={opt.value} onPress={() => setSortBy(opt.value)} style={[ws.sortRow, sortBy === opt.value && ws.sortRowActive]}>
                <Text style={[ws.sortTxt, sortBy === opt.value && { color: ORANGE, fontWeight: '700' }]}>{opt.label}</Text>
                {sortBy === opt.value && <Ionicons name="checkmark-circle" size={14} color={ORANGE} />}
              </TouchableOpacity>
            ))}
          </View>
          <View style={ws.sideCard}>
            ...
            {[
              { icon: 'add-circle-outline', label: 'Post a Room',   screen: 'PostRoom' },
              { icon: 'heart-outline',      label: 'Saved Rooms',   screen: null },
              { icon: 'chatbubble-outline', label: 'Contact Owner', screen: null },
            ].map(qa => (
              <TouchableOpacity key={qa.label} style={ws.quickAction} onPress={() => qa.screen && nav.navigate(qa.screen)}>
                <View style={[ws.quickIcon, { backgroundColor: '#fff7f0' }]}>
                  <Ionicons name={qa.icon} size={15} color={ORANGE} />
                </View>
                <Text style={ws.quickLabel}>{qa.label}</Text>
...
const ws = StyleSheet.create({
  root: {
    flex: 1, flexDirection: 'row', gap: 16, padding: 16,
    backgroundColor: '#f5f5f5', maxWidth: 1200, alignSelf: 'center', width: '100%',
  },
  sidebar: { width: 220, gap: 12 },
  ...
  sideCard: {
    ...
    shadowOffset: { width: 0, height: 2 }, elevation: 2, overflow: 'hidden', position: 'relative',
  },
  sideTitle: { fontSize: 11, fontWeight: '800', color: '#bbb', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 12 },
  ...
  tipTitle: { fontSize: 11, fontWeight: '800', color: '#15803d', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 10 },
